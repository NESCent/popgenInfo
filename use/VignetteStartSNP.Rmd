...
in R as a dataframe, and then  convert the SNP data file into a
...
To convert MydataR to genind object (adegent), we create the object locus which contains only genotypes
We remove the extra columns and we keep only 20 SNP (to make calculation faster).
It is also possible to convert Mydata1 into fstat file (Mydata2), or into pega object (Mydata3).
...
Populations are states. 
The fuction "basicstat" provides the observed heterozygosity (Ho), mean gene diversities within population (Hs), Fis, and Fst 
The function "boot.ppfis" provides confidence interval for Fis
The fuction "indpca"  does an PCA on the centered matrix of individual's allele frequencies
...
data](vignetteDifferentiationSNPbisGithub.html))
