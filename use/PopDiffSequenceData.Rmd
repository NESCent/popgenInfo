...
```{r fix_loci}
locusNames(beeData)
(locusNames(beeData) <- gsub(".fas", "", locusNames(beeData)))
```
...
```{r, eval = TRUE}
allData_dist <- dist.multidna(beeData, pool = TRUE)
amova(allData_dist ~ populations, data = strata(beeData.gid), nperm = 100)
```
