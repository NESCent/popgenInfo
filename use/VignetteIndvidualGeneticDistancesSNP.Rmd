...
##Section 2. Individual genetic distance: euclidean distance (`dist` {base R})

The unit of the observation is the individuals. x genotypes and 100 binary SNPs.
Ploidy : 2 The analysis is applied on allele frequency within individuals. We
can use the function `dist()` from base R which provides different options. We
will use the euclidean distance among vector of allele frequencies.
...
The option `pairwise.deletion = FALSE` in the command `distgen()` removes all
...
option `pairwise.deletion = TRUE` in the command `distgen()` allows you to keep
...
from euclidean distance or number of allelic differences between two individuals
...
Other R packages can also be used to estimate individual genetic distances like
gstudio for example. The individuals genetic distances can then be used in
analysis as Mantel tests to test for isolation by distance or more complex
analysis in landscape genetics to test for resistance by distance, cluster
analysis, spatial network... See related vignettes.
